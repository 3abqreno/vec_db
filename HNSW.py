...
class HNSW(object):
    ...
    def generate_database(num_vectors, dimension):
        # Initialize the HNSW class
        hnsw = HNSW(m=5, ef=1000)
        rng = np.random.default_rng(42)
        data = rng.random((num_vectors, dimension), dtype=np.float32)
        for i, vector in enumerate(data):
            hnsw.add(vector)
            if (i + 1) % 100 == 0:
                print(f"Added {i + 1} vectors to the HNSW index.")
        print("HNSW index built successfully!")
        return hnsw

    def cosine_distance(self, a, b):
        try:
            # Ensure inputs are 2D arrays for consistent behavior
            a = np.atleast_2d(a)
            b = np.atleast_2d(b)

            # Compute cosine similarity for batched inputs
            return np.dot(a, b.T) / (
                np.linalg.norm(a, axis=1, keepdims=True) * np.linalg.norm(b, axis=1)
            )
        except ValueError as e:
            print(f"Error: {e}")
            print(f"Shapes of a: {a.shape}, b: {b.shape}")
    ...
    def __init__(self, m=5, ef=200, m0=None, heuristic=True, vectorized=False):
        self.data = []
    ...
    def search(self, q, k=None, ef=None):
        ...
        # Extract indices and sort in the same way as the numpy argsort logic
        indices = [idx for _, idx in ep]
        scores = [-md for md, _ in ep]

        # Apply numpy-like sorting logic
        sorted_indices = [
            x
            for _, x in sorted(
                zip(scores, indices),
